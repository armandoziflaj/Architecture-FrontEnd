...
> **MANDATORY SYSTEM RULE:** Before proposing, planning, refactoring, or writing any code, read `./PROJECT_KNOWLEDGE.md` first — unless you already read it earlier in this same session. This mirrors the identical rule `GEMINI.md` gives other assistants working in this repo. Keep both docs true, not just one.
...
- `--font-mono` (`JetBrains Mono`) — index numbers, counters (`01 — 03`), metadata rows (location/year/size), language-switcher codes, dashboard stat values. Signals "data," not prose. Generic fallback: `monospace`, not `serif`. A prior lint auto-fix appended the wrong generic family here — double-check this whenever `--font-mono` appears in a diff.
...
- "No persona defined" / "no tool documentation" — this is an AI-directives file, not a chatbot spec or API reference.
- `GEMINI.md`, `PROJECT_KNOWLEDGE.md`, or a real component file reported as "not found" — verify with a file check first; they exist in this repo.
- "MUST" / "ANY" flagged as undefined acronyms — that's RFC-2119-style emphasis, not an acronym.
